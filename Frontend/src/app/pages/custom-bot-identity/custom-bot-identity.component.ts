...
export class CustomBotIdentityComponent implements OnInit {
  ...
  deactivate(): void { const guildId = this.store.selectedGuild()?.id; if (!guildId) return; this.run('returnToPlatform', this.api.deactivateCustomBot(guildId), result => { this.identity.set(result.identity); this.diagnostics.set(null); this.toast.success(this.i18n.translate('customBotIdentity.deactivated')); }); }
