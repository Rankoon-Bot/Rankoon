        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                await database.MemberXp.Indexes.CreateManyAsync([
                    ...
                    new CreateIndexModel<MemberXp>(Builders<MemberXp>.IndexKeys.Ascending(x => x.GuildId).Descending(x => x.TotalXp).Ascending(x => x.NormalizedDisplayName).Ascending(x => x.UserId), new CreateIndexOptions { Name = "guild_xp_name_user" }),
                    new CreateIndexModel<MemberXp>(Builders<MemberXp>.IndexKeys.Ascending(x => x.GuildId).Ascending(x => x.IsCurrentMember).Descending(x => x.TotalXp).Ascending(x => x.NormalizedDisplayName).Ascending(x => x.UserId), new CreateIndexOptions { Name = "guild_current_xp_name_user" })
                ], stoppingToken);
                ...
                await database.VoiceActivities.Indexes.CreateManyAsync([
                    ...
                    new CreateIndexModel<VoiceActivityDay>(Builders<VoiceActivityDay>.IndexKeys.Ascending(x => x.GuildId).Ascending(x => x.UserId).Ascending(x => x.DayStartUtc).Ascending("session_cursors.session_id"), new CreateIndexOptions { Name = "guild_user_day_session" }),
                    new CreateIndexModel<VoiceActivityDay>(Builders<VoiceActivityDay>.IndexKeys.Ascending(x => x.ProjectionStatus).Ascending(x => x.ProjectionLeaseExpiresAtUtc).Ascending(x => x.UpdatedAtUtc), new CreateIndexOptions<VoiceActivityDay> { Name = "open_voice_projection", PartialFilterExpression = new BsonDocument("projection_status", new BsonDocument("$in", new BsonArray { "Pending", "Projecting" })) }),
                    new CreateIndexModel<VoiceActivityDay>(Builders<VoiceActivityDay>.IndexKeys.Ascending(x => x.GuildId).Ascending(x => x.DayStartUtc), new CreateIndexOptions { Name = "guild_period" })
                ], stoppingToken);
                await database.VoiceLedgerMigrationStates.Indexes.CreateOneAsync(new CreateIndexModel<VoiceLedgerMigrationState>(Builders<VoiceLedgerMigrationState>.IndexKeys.Ascending(x => x.Phase).Ascending(x => x.UpdatedAtUtc), new CreateIndexOptions { Name = "phase_updated" }), cancellationToken: stoppingToken);
