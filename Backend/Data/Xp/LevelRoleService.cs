...
public sealed class LevelRoleService(IGuildDiscordContextResolver discord, IXpService xp)
{
    public async Task<LevelRoleSynchronizationResult> SynchronizeAsync(ulong guildId, ulong userId, CancellationToken cancellationToken = default)
    {
        ...
        var guild = (await discord.ResolveAsync(guildId, cancellationToken))?.Guild; var member = guild?.GetUser(userId); if (guild == null || member == null) return new(added, removed, failed, alreadyPresent);
        ...
        foreach (var definition in deserved.Values)
        {
            ...
            if (member.Roles.Any(memberRole => memberRole.Id == definition.RoleId)) { alreadyPresent.Add(change); continue; }
