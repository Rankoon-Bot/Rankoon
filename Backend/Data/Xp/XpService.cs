...
public sealed class XpService(RankoonDbContext database, IReportWriter reports, ILogger<XpService> logger) : IXpService
{
    ...
    public async Task<bool> GrantAsync(ulong guildId, ulong userId, string displayName, string source, decimal amount, string key, ulong? channelId = null, CancellationToken cancellationToken = default)
    {
        ...
        var preference = await database.MemberLeaderboardPreferences.Find(x => x.GuildId == guildId && x.UserId == userId).FirstOrDefaultAsync(cancellationToken);
        var update = Builders<MemberXp>.Update
            .SetOnInsert(x => x.GuildId, guildId).SetOnInsert(x => x.UserId, userId)
            .SetOnInsert(x => x.PublicLeaderboardVisible, preference?.PublicVisible ?? true)
            .Set(x => x.DisplayName, displayName).Set(x => x.UpdatedAt, now)
            .Set(x => x.IsCurrentMember, true)
            .Inc(x => x.EarnedXp, amount).Inc(x => x.TotalXp, amount);
